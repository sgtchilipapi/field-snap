...
export function buildDocumentClassificationPrompt(
  ...
  return [
    ...
    "Routing hints: retail receipts -> receipts; vendor bills asking the business to pay -> vendor_bills; invoices issued by the business to customers -> customer_invoices; worksite or material photos -> job_photos; signed agreements -> contracts; permits -> permits; change orders -> change_orders; equipment-related documents -> equipment.",
    "For invoice-like documents, compare the visible issuer/payee/vendor/seller name with business_name. If it matches business_name, classify it as customer_invoice and target customer_invoices; if a different vendor issued it and the business is the bill-to/customer, classify it as vendor_bill and target vendor_bills.",
    "For invoice-like documents, set vendor_or_party to the visible issuer/payee/vendor/seller name used for that business-name comparison, not merely the bill-to/customer name.",
