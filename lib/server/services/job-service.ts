...
export const createJobSchema = z
  .object({
    ...
    custom_category_name: z.string().trim().max(120, "Category name must be 120 characters or fewer.").optional(),
    client_name: z.string().trim().min(1, "Client name is required.").max(120, "Client name must be 120 characters or fewer."),
...
async function createCustomCategoryForBusiness(input: {
  ...
  const existingByName = await findCategoryForBusinessByName(input.businessId, input.name);
...
async function requireDriveConnectionForBusiness(businessId: string) {
  ...
  if (!connection || connection.status !== "active" || !connection.access_token_encrypted) {
    throw new JobServiceError("An active Google Drive connection is required.", "drive_unavailable");
  }
...
export async function createJobForBusiness(input: {
  ...
  const { accessToken } = await requireDriveConnectionForBusiness(input.businessId);

  let category: CategoryRow | null = null;

  if (customCategoryName) {
    if (!ownerDetails.business.drive_root_folder_id) {
      throw new JobServiceError("The business Drive root folder is not available.", "drive_unavailable");
    }
    ...
    category = await getCategoryForBusiness(input.businessId, parsed.category_id);
  }

  if (!category || !category.drive_folder_id) {
    throw new JobServiceError("The selected category is not available.", "invalid_category");
  }
  ...
  try {
    ...
    const inProcessFolder = folderRecords.find((folder) => folder.folderKey === "in_process");
    const needsReviewFolder = folderRecords.find((folder) => folder.folderKey === "needs_review");
...
export async function updateJobForBusiness(input: {
  ...
  const jobName = parsed.job_name.trim();
  const address = normalizeAddress(parsed.address);
  const category = await getCategoryForBusiness(input.businessId, parsed.category_id);
